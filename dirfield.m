...
%end
yp=feval(fv,tm,ym); 
s = 1./max(1/dt,abs(yp)./dy)*0.40;
k = quiver(tval,yval,s,s.*yp,0,'.r');
l = quiver(tval,yval,-s,-s.*yp,0,'.r');

set(k,"maxheadsize",0.53);
set(l,"maxheadsize",0.53);

axis([tval(1)-dt/2,tval(end)+dt/2,yval(1)-dy/2,yval(end)+dy/2],"equal");
