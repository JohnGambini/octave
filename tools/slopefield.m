function slopefield(f,tval,yval)
% dirfield(f, t1:dt:t2, y1:dy:y2)
%   
%   plot a slope field for first order ODE y' = f(t,y)
...
yp=feval(fv,tm,ym); 
s = 1./max(1/dt,abs(yp)./dy)*0.1;
hold on;

k = quiver(tval,yval,s,s.*yp,0.35,'.r','linewidth', 1);
l = quiver(tval,yval,-s,-s.*yp,0.35,'.r','linewidth',1);

set(k,"maxheadsize",0);
set(l,"maxheadsize",0);

plot(tm,ym,'.','markersize',12,'color', 'b');
