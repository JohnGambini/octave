...
ud.ssize = ssize;
  ...
  ud.c = system;	% Changed values.
  ud.o = system;	% Original values.
  ud.h = system;	% This will be the handles in the
                        % setup window.
  ud.fieldtype = 'lines';
  ud.npts = npts;  
  ud.style = style;
  ...
  dfset = figure('name','DFIELD6 Setup','numb','off',...
  ...
  frsep = 0;    	% Separation between frames.
  ...
  ud.h.der=uicontrol('pos',[eqleft+varw+equalw derbot eqlength texth],...
      ...
      'call',der,'visible','off','backgroundcolor',tcolor);
  
  eq(3) = uicontrol('style','text',...
      'horizon','right','string','The independent variable is ',...
  ...
  ud.h.tname = uicontrol('pos',[tvarl tbot varw texth],'style','edit',...
     'horizon','left','string',ud.o.tname,'call',tname,...
     'visible','off','backgroundcolor',tcolor);
  ...
  paratitw = ext(3);
  pos = [eqleft pbot-fudge paratitw texth];
  set(paratit,'pos',pos);
  pleft1 = eqleft + paratitw + 3;
  ...
  butt(1) = uicontrol('style','push',...
      'pos',qwind,...
      'string','Quit','call','dfield6(''quit'')','visible','off');
  
  butt(2) = uicontrol('style','push',...
      'pos',rwind,...
      'string','Revert','call','dfield6(''revert'')','visible','off');
  
  butt(3) = uicontrol('style','push',...
      'pos',pwind,...
      'string','Proceed','call','dfield6(''proceed'')','visible','off');
  ...
  meproceed = uimenu(mefile,'label','Proceed','call',...
      'dfield6(''proceed'')','separator','off',...
      'accelerator','G','pos',1);
  merevert = uimenu(mefile,'label','Revert','call',...
      'dfield6(''revert'')','separator','off','pos',2);
  set(mepset,'vis','on','pos',3);
  set(meprev,'vis','on','label','Page Pre&view...','pos',4);
  set(meexp,'vis','on','pos',5,'separator','off');
  merestart = uimenu(mefile,'label','Restart DFIELD6','call',...
      'dfield6(''restart'')','separator','on','pos',6);
  mequit = uimenu(mefile,'label','Quit DFIELD6','call',...
      'dfield6(''quit'')','separator','on','pos',7);
  ...
  meclrp = uimenu(meedit,'label','Clear parameters',...
      'call',['ud = get(gcf,''user'');h = ud.h;',...
	'set([h.pname1,h.pname2,h.pval1,h.pval2],''string'','''');'],...
      'accelerator','N');
  ...
  meclrall = uimenu(meedit,'label','Clear all',...
      'call',['ud = get(gcf,''user'');h = ud.h;',...
	'set(h.wind,''string'','''');',...
	'set([h.xname,h.der,h.tname,h.der,h.pname1,h.pname2,h.pval1,h.pval2],''string'','''');'],...
      'accelerator','A',...
      'separator','on');
...
elseif strcmp(action,'proceed')
  ...
  if (sud.flag == 1)
	...
	xderivstr = dfield6('paraeval',pname1,parav1,derivstr);
	...
	xderivstr = dfield6('paraeval',pname2,parav2,derivstr);
	if ~strcmp(xderivstr,derivstr)
	  pflag(2) = 1;
	  derivstr = xderivstr;
	end
...
% $$$       set(findobj(tmenu,'label','&Rotate 3D'),'vis','off');
% $$$       set(findobj(tmenu,'label','Zoom &Out'),'vis','off');
% $$$       set(findobj(tmenu,'label','&Zoom In'),'vis','off');
% $$$       set(findobj(tmenu,'label','Show Le&gend'),'vis','off');
% $$$       set(findobj(tmenu,'label','Enable &Plot Editing'),'vis','off');
% $$$       set(findobj(tmenu,'label','Show &Toolbar'),...
% $$$ 	  'call','dfield6(''showbar'')')
      ...
      set(findobj(emenu,'label','&Undo'),'separator','on',...
			'pos',7);
      set(findobj(emenu,'label','Cu&t'),'pos',8);
      set(findobj(emenu,'label','&Copy'),'pos',9);
      set(findobj(emenu,'label','&Paste'),'pos',10);
...
elseif strcmp(action,'solution')
  ...
  if intplus(2)>intplus(1)
    ...
    switch settings.solver
     case 'Dormand-Prince'
      [tp,xp] = dfdp45(dfcn,intplus,initpt(2),disph);
      
     case 'Euler'	
      [tp,xp] = dfeul(dfcn,intplus,initpt(2),disph);
      
     case 'Runge-Kutta 2'	
      [tp,xp] = dfrk2(dfcn,intplus,initpt(2),disph);
      
     case 'Runge-Kutta 4'	
      [tp,xp] = dfrk4(dfcn,intplus,initpt(2),disph);
  ...
  if intminus(2) < intminus(1)
    ...
    switch settings.solver
     case 'Dormand-Prince'
      [tm,xm] = dfdp45(dfcn,intminus,initpt(2),disph);
      
     case 'Euler'	
      [tm,xm] = dfeul(dfcn,intminus,initpt(2),disph);
      
     case 'Runge-Kutta 2'	
      [tm,xm] = dfrk2(dfcn,intminus,initpt(2),disph);
      
     case 'Runge-Kutta 4'	
      [tm,xm] = dfrk4(dfcn,intminus,initpt(2),disph);
...
elseif strcmp(action,'kbd')
  ...
  speccall = [
    'ud = get(gcf,''user'');',...
	'if get(gcbo,''value''),',...
	'  set([ud.t0,ud.t,ud.tf],''vis'',''on'');',...
	'else,',...
	'  set([ud.t0,ud.t,ud.tf],''vis'',''off'');',...
	'end'];
  ...
  set([kbd.t0,kbd.t,kbd.tf],'vis','off')
  set(findobj(ppkbd,'type','uicontrol'),'units','normal');
  figure(ppkbd)
...
elseif strcmp(action,'solvset')
  ...
  setfig = figure('name','DFIELD6 Solver settings','numb','off',...
      'tag','DFIELD6','vis','off');
...
elseif strcmp(action,'settings')
  ...
  data.rad(3) = uicontrol('style','radio',...
      'pos',[rbl rb1b rbw ht],...
      'string','None','value',rval3,'max',3,'visible','off');
  ...
  npts = uicontrol('style','edit',...
      ...
      'string',npts,'call',callnfpts,'visible','off');
